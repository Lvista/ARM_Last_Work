...
void lcd_init(void)
{
    ...
    else if (lcddev.id == 0x1963)
    {
        ...
        lcd_ssd_backlight_set(60); /* 背光设置为最亮 */
    }
...
void lcd_show_string_my(uint16_t x, uint16_t y, uint8_t size, char *p, uint16_t color)
{
    lcd_show_string(x, y, strlen(p) * size, size, size, p, color);
}
